---
title: Citadel Small Base Brush Review: Tiny Tool, Big Impact on Bases (2.0)
date: 2026-04-08
tags:
  - miniatures
  - painting
  - brushes
  - citadel
  - hobby
  - review
---

# Citadel Small Base Brush Review: Tiny Tool, Big Impact on Bases

![Citadel Small Base Brush](assets/images/citadel-small-base-brush.jpg)

## Introduction
If you’ve spent enough time around hobby shelves to know your Valhallan blue from your dry-brushing disaster, you’ll recognize that the right brush can turn a good paint job into something people stare at across the internet and whisper, is that base actually finished? The Citadel Small Base Brush is the kind of tool that promises precision for tiny basing work without forcing you to break out a microscope and a prayer. It is marketed as a compact, dedicated tool for base-coating. But does it deliver? Is it the unsung hero of basing, or merely a cute accessory for folks who insist on base texture that looks like a postcard from a tropical island rather than a swamp you removed from a sink drain? In this review we subject the Citadel Small Base Brush to the Geeknite test: puns, paint, and actually finishing those little bases without surrendering to the brush gods.

For context, basing tends to be the most frustrating part of painting for many hobbyists. It’s where you realize that you can paint the miniature to a four-star standard, but the base makes or breaks the whole presentation. A good base can echo a miniature’s story, whether it’s a blasted alien planet, a sci-fi ship deck, or a forest floor thick with tufts. The right base brush should help you apply base coats efficiently, avoid gouges, reach tight corners around rubble, and do so without blood sacrifices to the bristle gods. The Citadel Small Base Brush aims to be that tool. This review will skate through the brush’s design, how it actually performs, its longevity, and whether it’s worth your paint budget.

If you’re more of a visual learner, we’ve included practical tips, comparisons with other brushes, and a little humor to keep the vibes high as you contemplate your basing strategy.

> For you completionist readers: this post includes links to other Geeknite posts about brushes and basing techniques. Check the recommended reads at the end; you’ll be painting like a pro and still have time to binge a sci-fi show.

## What is the Citadel Small Base Brush?
The Citadel Small Base Brush is a compact, pointy-tipped brush designed specifically for base layers and small, tight areas on miniature bases. It’s commonly marketed as a dedicated base brush that helps you lay down even coats on base textures—things like gravel, rubble, rusted metal, tufts, and ground cover. The idea is simple: a smaller ferrule and a milder point to reach tricky spots without inadvertently painting the model’s feet or legs. It’s the brush you pull out when you’re trying not to ruin that perfect base terrain you’ve spent an hour planning in your head.

From a quick look, you’ll notice a few hallmark features: a compact head size, a fine but resilient tip, and a handle that isn’t long enough to be a hazard when you’re churning around a tabletop. Citadel brushes are known for color-coded handles and slightly stiffer bristles than budget alternatives, which makes them appealing to hobbyists who want control. The Small Base Brush sits in the base coating and details tier of Citadel’s brush lineup, sitting between the larger base brushes and the very small dry brushes. It’s not meant for lay-in highlights or heavy dry-brushing; it’s meant for crisp base coats and careful, precise, controlled application.

If you’re curious about what base brushes exist beyond Citadel, you’ll often see comparisons with Army Painter’s base brushes and various sable-hair options from other brands. The goal is to decide what your workflow looks like: do you prefer synthetic bristles that hold a lot of paint or a more responsive bristle that allows you to pull paint into crevices carefully? The Small Base Brush claims to be the right choice for the first step, the part that determines whether your base has depth, texture, and a sense of place.

![Close-up of the Citadel Small Base Brush bristles](assets/images/citadel-small-base-brush-closeup.jpg)

## Build quality and design
### Bristle material and feel
The Small Base Brush uses a synthetic bristle blend typical of Citadel’s brush line. The bristles are relatively stiff at first use, which translates to a nice level of control when applying base coats to textured surfaces. The bristle stiffness is a functional feature: you can press lightly to flood a textured base with paint or give a gentle nudge to work paint into crevices. Over time, some hobbyists report bristles softening with use, particularly if you’re washing with abrasive cleaners. The take-home: treat it well, and it will treat you well back, like a loyal companion digitizing your painting workflow rather than a rogue hairball in your wash basin.

### Ferrule and handle
The Small Base Brush sits on a small, sturdy ferrule. It’s not the most luxurious thing you’ll own, but it doesn’t feel cheap. The grip is designed to minimize fatigue during longer basing sessions, though you’ll want to avoid the urge to twist it into a makeshift signal flare while you’re reaching for those hard-to-reach base corners. The overall ergonomics are decent: short, balanced, and designed to be swapped in and out of a painting session without requiring a chiropractor appointment afterward.

### Size and reach
As advertised, it’s a compact tool. The head is small enough to reach around tiny details within the base, but not so tiny that it becomes a complete one-trick pony. It’s ideal for terrain textures like rubble, cracks, lacunae of soil, and the edges around a base’s rim where you don’t want paint trailing onto the model. If you’re painting large flat base sections, you might want something bigger for speed, but for precise work, this brush is a comforting ally.

## Performance on bases
### Base coats that behave
The primary test for a base brush is whether it lays down a clean, even coat over varied textures. The Citadel Small Base Brush is surprisingly good at this. It holds enough paint to cover textured areas without flooding, and the fine tip lets you avoid accumulating too much paint in crevices. If you’re working with textured bases that have tiny pebbles, sand, or crystal gravels, you can deliver a base color evenly across the surface without the base turning into a stubborn, mottled mess.

One practical tip: when you dip this brush into paint, avoid overloading. The Small Base Brush isn’t a glide-through-everything tool; it’s more of a precise instrument. A light touch helps maintain control and reduces the risk of paint pooling between the base texture and the base rim. You’ll want to keep your paint at a comfortable consistency—some machinists call this the viscosity of a good coffee, i.e., not too thick, not too thin.

### Reaching tricky edges
The small head makes it easier to reach the edge of a base, particularly around jagged rocks or placed textures where you want an even base color without bleeding onto the miniature. In practice, the brush’s point allows you to guide paint along crevices while keeping a safe distance from the model’s lower limbs. It’s not magic, but it’s a notable improvement over using a larger brush for those micro-areas. If your bases include complex textures, the Small Base Brush gives you the control to keep edges clean and base colors within the desired zones.

### Blending and consistency
If you’re tempted to use the brush for blending, you’ll likely find better results using it for base coats and one-dip texture work rather than for soft gradient blends. It’s not designed as a blender brush; for smooth gradients, you’ll want a softer tip with more bounce. That said, you can achieve subtle transitions along the rim of a base by applying a light wash with this brush and then carefully pulling it toward the edges with the same instrument. It does not replace a dedicated blending brush, but it does offer a surprising amount of versatility for a brush of its size.

## Bristle quality and longevity
### Early-life behavior
In the first few uses, you’ll notice the bristles maintain their stiffness and shape well. The brush doesn’t shed like a cat on a hairbrush but, like all brushes, you may see a few stray fibers after aggressive washing. A good rule of thumb: avoid pulling fibers out manually; instead, wash with warm soapy water, reshape while damp, then air-dry with the bristles pointing downward. This helps maintain the brush’s tip shape and prevents bristle fraying that could compromise detail control over time.

### Long-term durability
Long-term durability is always a concern with any brush, especially for hobbyists who treat their tools as investments in their craft. Citadel brushes, including the Small Base, tend to hold up reasonably well with proper care. If you paint daily and scrub under a faucet, you may see minor bristle loss. If you’re a casual painter who cleans more gently and stores the brush correctly, you’ll likely enjoy this brush for months or longer. The key to longevity is avoiding harsh solvents and storing the brush upright, bristles up, rather than stuffing it into a drawer with dirty water pooled at the ferrule.

## Care and maintenance
### Cleaning routine
After each use, rinse the brush in lukewarm water until the water runs clear. Use a mild soap if you’ve been working with pigments that stain bristles (think browns and greens) and gently work the bristles with your fingers to avoid tangling. Never leave a brush soaking in paint or solvent for extended periods once you’re done—this can bend the bristles and degrade performance. Drying should be done with the brush laid flat or with the bristles pointed downward to prevent water from seeping into the ferrule.

### Storage tips
Store the brush in a clean, dry place away from dust. A brush holder or rack that keeps the bristles in a natural shape is ideal. If you’re traveling with miniatures, consider a protective cap for the brush head, though many hobbyists prefer to keep it open, citing flexibility and quick access as more important in field sessions.

### Replacement and maintenance mindset
If you notice the bristles shedding or the head losing its shape after many uses, it might be time to replace or diversify your brush selection. The Small Base Brush isn’t an inherently fragile tool, but it does have a finite lifespan, especially under heavy use. The upside is that its price point is reasonably accessible, so you won’t cry too much if you decide to rotate in another brush for a month while you still rely on the Small Base for the heavy lifting.

## Comparisons with other base brushes
### Citadel vs. Army Painter base brushes
When pitted against Army Painter’s base brushes, the Citadel Small Base shows its strengths in precision work. Army Painter’s equivalents tend to have a different feel—some hobbyists praise the slightly stiffer bristles for controlling paint in rough textures. If you’re someone who likes to control the distance between the brush and your base surface, Citadel’s offering can feel more predictable, especially on fine textured bases where you want to avoid oversaturation.

### The big-brush alternative
If you absolutely need to stain a larger surface quickly, a bigger base brush gets the job done but sacrifices precision. The Small Base Brush finds a happy middle ground: it’s small enough to navigate tricky base textures but not so small that you lose coverage on flatter areas. It’s a good candidate to pair with a larger base brush in your toolkit for different stages of basing and texturing.

## Price and value
Prices vary by region and retailer, but in general, you’ll find the Citadel Small Base Brush priced in the mid-range for hobby tools. The value proposition is not just get the job done but get the job done with less frustration. If your basing routine involves playing with texture powders, sand, and small stones, you’ll likely feel you got more control per dollar than you would with a generic, larger brush used for everything. It’s a reasonable addition to a starting hobby budget or an essential tool for more advanced basing schemes.

## Tips and techniques for using the Small Base Brush
- Start with a proper paint consistency. A slightly thinner base coat helps you avoid clogging into texture cracks.
- Use a light touch. Let the bristles do the work; you don’t need to press hard to achieve coverage.
- Angle the brush to reach edges with precision. A shallow angle helps you lay color into nooks without splashing onto the model.
- Work in stages: apply base color, then adjust with a second pass if needed. Don’t overwork a single pass; you’ll lose texture and life in the base.
- Combine with washes or dry brushing for depth. The brush can help with a restrained base wash that darkens crevices and emphasizes terrain without pooling in flat areas.
- Clean promptly after use, especially if you’re using pigments like earth tones that cling to bristles. This reduces staining and preserves the brush’s life.

## Common criticisms
Like any tool, the Citadel Small Base Brush isn’t perfect. A few common criticisms from the hobby community include:
- The bristles may shed slightly over time with heavy use. This is not unusual for synthetic bristle brushes, but it can be annoying if you’re chasing a pristine bristle setup.
- Some painters desire a slightly longer handle for better leverage on larger basing textures. If you’re mashing into rugged terrain with a lot of reach required, a longer-handled variant might feel more comfortable.
- It’s not the best for large flat areas where you’d prefer speed to precision; you’ll still want a larger brush for big base textures or sidewalks of base material.

## Who should buy this brush?
- Beginners who want a forgiving tool specifically for bases, with a level of control that helps reduce the oops, I painted the miniature base accidents.
- Intermediate painters who want a dedicated tool for precise base layers and tight corners around the base rim.
- Hobbyists who enjoy layering textures and need a brush versatile enough to handle little edges without compromising the miniature’s main figure.

If you’re someone who rushes through bases because you’re more excited about the armor side of the miniature or the dramatic paint job on the torso, this brush can be a gentle reminder to slow down and pay attention to the base as part of the whole diorama.

## Alternatives worth considering
- A larger base brush for faster coverage when the base has broad, flat terrain. If you pair it with the Small Base Brush, you can optimize for speed and precision.
- A softer, sable-hair alternative for blending base colors and textures with a lighter touch.
- A dedicated dry brush for rough texture highlights if you intend to do very specific base texture work beyond base coats.

## Final verdict
The Citadel Small Base Brush is a practical, well-made tool that earns its keep by delivering precise base coverage without excessive fuss. It’s not a one-tool wonder that replaces your entire brush collection, but it is a solid, dependable member of your basing toolkit. If you’re working on bases that involve sand, rubble, or textured ground and you want to minimize paint bleeding onto the model, this brush is a reliable ally. It offers good control, reasonable longevity with proper care, and enough versatility to justify its place in your brush roll.

For basing enthusiasts who want to level up their game without sinking into a sea of conflicting brush types, this is a sensible purchase. It aligns with Geeknite’s ethos: practical tools that enhance your hobby without turning painting into a full-time job.

## Where to buy and external reading
- Official product page: https://www.games-workshop.com/en-GB/Citadel-Base-Brush
- A quick guide on brush care and maintenance: https://www.paintingminiatures.org/brushtips
- Read more on painting brushes and technique: [How to select brushes]({{ post_url 'how-to-select-paint-brushes' }})
- Explore more on basing ideas and textures: [Basing ideas: textures and groundwork]({{ post_url 'basing-textures-ideas' }})
- Additional reading: [Advanced basing techniques]({{ post_url 'advanced-basing-techniques' }})

## Final notes on care and usage
If you treat the Citadel Small Base Brush with a bit of respect, you’ll likely find it more useful than you expect. It’s not a flashy tool meant for grand dramatic effect; it’s the kind of tool that quietly helps you keep your bases looking deliberate and well-placed. It’s a friendly reminder that sometimes the smallest tool in the box can yield the most satisfying results when you’re building a miniature world from the ground up.

## Callouts and community insights
- On forums and social posts, many hobbyists appreciate the brush for its reach into base textures and its ability to control edge painting around bases. The consensus is often that the Small Base Brush shines when you’re working with varied textures and you want crisp lines, especially around the rim where the model touches the base.
- If you’re a completionist who likes everything to look sharp, consider pairing this brush with a small detail brush for touch-ups around the base’s edges and to refine the base’s final appearance.

## Links to related Geeknite content
- Brush roundups and comparisons: [Read more about brush comparisons]({{ post_url 'brush-roundup-2025' }})
- Basings that pop: [Ground textures and basing materials]({{ post_url 'ground-textures-basing' }})
- A guide to painting minis in a sci-fi setting: [Sci-fi palette playbook]({{ post_url 'sci-fi-palette-playbook' }})

## Final recommendation
If you’re starting a basing project or you’re trying to tighten up your base coats without covering your miniature, the Citadel Small Base Brush is a solid choice. It delivers precision, control, and enough versatility to justify its place in your brush lineup. It may not replace your larger brushes or your favorite blending tools, but it will likely improve your basing workflow and reduce the number of do-overs you need to do around the base rim.

**Buy via our affiliate link: https://geeknite.shop/affiliate/citadel-small-base-brush**